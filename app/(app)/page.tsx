...
export default function Home() {
  ...
  const canAutoSend = canUse(clinic, 'auto_outreach');
  ...
  function handleSendLead(patientId: string, message: string) {
    const id = store.createOutreach(patientId, message);
    ...
    <div className="body">
      <main className="main">
        <div className="feed">
          ...
          {!canAutoSend && actionable.length > 0 && (
            <div style={{ background: 'var(--accent-soft)', border: '1px solid var(--accent)', borderRadius: 14, padding: '10px 14px', marginBottom: 14, fontSize: 12.5, color: 'var(--accent)' }}>
              💡 {actionable.length} מטופלים מחכים להחזרה — בתוכנית המתקדמת המערכת שולחת את ההודעות בעצמה (באישורך).
            </div>
          )}
