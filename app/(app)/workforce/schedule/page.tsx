...
export default function SchedulePage() {
  ...
  const jobData = activeJobs.map(job => {
    ...
    const gaps = requiredShifts.filter(s => !coveredShifts.has(s as ShiftType)) as ShiftType[];
    ...
    return { job, requiredShifts: requiredShifts as ShiftType[], coveredShifts, respondedCandidates, gaps, suggestions };
  });
          ...
          {jobData.map(({ job, requiredShifts, coveredShifts, respondedCandidates, gaps, suggestions }) => (
            <div key={job.id} className="sch-card">
              ...
              <div className="sch-card-head">
                ...
                <Link href={`/hiring/jobs/${job.id}`} className="sch-job-link">פרטים ←</Link>
              </div>
                    ...
                    <Link key={c.id} href={`/candidate/${c.id}?job=${job.id}`} className="sch-suggest-row">
                      ...
                      <div className="sch-suggest-info">
                        ...
                        <div className="sch-suggest-facts">
                          {c.availability.shifts.filter(s => gaps.includes(s as ShiftType)).map(s => SHIFT_HE[s]).join(' · ')}
